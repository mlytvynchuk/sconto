      ...
      <div className="like-list">
        ...
        <div className="list-edit">
          {this.props.likes.length > 0 ? (
              ...
              <li className="ticket" key={item.id}>
                <div className="d-flex">
                  <div className="display-left">
                    <LightBox1 button={<h4>{item.discount.cafe}</h4>}>
                      {console.log(item)}
                      <DiscountDetails
                        id={item.discount.id}
                    ...
                    <p>{item.discount.location}</p>{" "}
                  </div>
                  <div className="display-right">
                    <img src={settings.DOMAIN + item.discount.image} alt="" />
                    <button
                      className="btn"
                      onClick={() => this.props.deleteFromLikes(item.id)}
                    >
                      {" "}
                    </button>{" "}
                  </div>
            ...
            <div className="text-center">Ви нічого ще не вподобали</div>
          )}
        </div>
      </div>
    );
